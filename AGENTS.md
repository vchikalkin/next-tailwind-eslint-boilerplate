...
## Output Optimization & Token Saving (Strict)
- **No Prolixity**: Полностью исключи приветствия, вводные фразы ("Вот ваш код:") и вежливые заключения. Сразу выдавай результат.
- **Diff-Only / Code Splices**: Никогда не переписывай весь файл целиком, если изменения точечные. Показывай только измененный фрагмент, конкретную функцию или используй плейсхолдеры `// ... существующий код`.
- **Brief Explanations**: Текстовые пояснения к коду должны быть не длиннее 1–2 емких предложений. Если код очевиден — не поясняй его вообще.